...
class Profile(models.Model):
    ...
    root_admin = models.OneToOneField(User, on_delete=models.SET_NULL)
    industry_category = models.OneToOneField(IndustryCategory, on_delete=models.SET_NULL)
...
class Account(models.Model):
    company_profie = models.OneToOneField(Profile, on_delete=models.SET_NULL)
    date_create = models.DateField(auto_now=True)
    subscription_plan = models.ForeignKey(SubscriptionPlan, on_delete=models.CASCADE)
    add_users = models.ManyToManyField(User)


class SubscriptioPlan(models.Model):
    user_limit = models.IntegerField(null=True, blank=True)
    start_date = models.DateTimeField(auto_now=True)
    duration = models.DurationField(null=True, blank=True)
    subscription_package = models.ManyToManyField(SubscriptioPackage)
